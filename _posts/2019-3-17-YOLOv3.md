...
![](http://latex.codecogs.com/gif.latex?\begin{aligned}b_{w} & = a_{w}e^{t_{w}} \\b_{h} & = a_{h}e^{t_{h}}\end{aligned})
其中，![](http://latex.codecogs.com/gif.latex?a_{w})和$a_{h}$为anchor box的宽和高，$t_{w}$和$t_{h}$为bounding box直接预测出的宽和高，$b_{w}$和$b_{h}$为转换后预测的实际宽和高，这也就是最终预测中输出的宽和高。你可能会想，这个公式这么麻烦，为什么不能用$\begin{aligned}
